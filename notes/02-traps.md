...
| 20 | A decorator is an attribute — metadata a framework reads | A decorator is a function that runs at definition time and can replace what it decorates. It is middleware, not metadata, and Chapter 5's headline. **Delivered: Ch. 5 SS5.2**, elicited by the transcript that shows a line recorded before anything is called | Ch. 5 |
| 21 | A comprehension is LINQ | A list comprehension is eager where `IEnumerable` is deferred; a generator expression is deferred **and single-pass**, where a LINQ query re-runs on every walk. Materialise once, on purpose. **Delivered: Ch. 5 SS5.5** | Ch. 5 |
| 22 | A closure in a loop captures the loop variable's value | It captures the variable, late-bound; every closure sees the last value. Bind with a default argument or `functools.partial`. Ruff's B023 catches it, which makes this the one trap in the chapter the toolchain finds for you. **Delivered: Ch. 5 SS5.4** | Ch. 5 |
| 23 | `s += piece` in a loop is fine, strings are strings | **Corrected by measurement, and the entry as first written was wrong.** On CPython the plain loop is LINEAR: the interpreter carries a specialisation, `BINARY_OP_INPLACE_ADD_UNICODE`, that resizes in place when the left operand has no other reference. It is quadratic again the moment anything else holds a reference — a list, a closure, an attribute — with nothing at the call site changed. `"".join(pieces)` has no bad case. **Delivered: Ch. 5 SS5.5.1**, measured by `code/measure/ch05_concat.py` and committed as bounds | Ch. 5 |
| 24 | A generator is `yield return` | It is, including the `finally` that runs on early exit, which is `IEnumerator.Dispose`. It also has `send()`, `close()` and a return value carried out — a coroutine before `async` existed, which is why `Generator` has three type parameters where `IEnumerable` has one. **Delivered: Ch. 5 SS5.6** | Ch. 5 |
