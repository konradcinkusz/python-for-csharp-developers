...
\emph{import}, a nie pierwsze użycie; a różnicą, która gryzie, nie jest
bezpieczeństwo wątkowe, bo CPython bierze blokadę na moduł na czas
wykonywania ciała i wykrywa własne zakleszczenia, tylko to, że cykl
pozwala drugiemu modułowi zobaczyć ten w połowie zbudowany, o czym jest
\S\ref{sec:cycles}.
\end{csbox}
...
\begin{csbox}
\code{-m} nie ma dokładnego odpowiednika w C\#, i to właśnie jest użyteczne.
W .NET to, względem czego rozwiązuje się zestaw, jest zadeklarowane \dash{}
przez projekt, a w czasie działania przez plik deps zapisany obok \dash{}
więc katalog, w którym akurat stoisz, nie zmienia tego, jaki kod się
ładuje. W Pythonie zmienia, bo \api{sys.path} zaczyna się od katalogu
wybranego przez sposób uruchomienia procesu. Układ \code{src/} z
rozdziału~\ref{ch:packaging} istnieje po to, by przestało to mieć
znaczenie, a ta sekcja jest mechanizmem pod nim.
\end{csbox}
