...
instalowały dokładnie te same \val{e2.packages} dystrybucji bez niczego do
rozwiązania, na przypiętym interpreterze, każde z prywatnym cache'em pakietów
...
żaden cache nie usuwa z niej niczego, dlatego ciepły wynik \code{pip}-a mieści
się w zaokrągleniu jego zimnego. \pkg{uv} zapisuje ciepłe instalacje,
linkując z cache'u zamiast kopiować, więc gdy koła są już lokalne, zostaje
prawie nic do zrobienia.
...
\val{e2.uv.cold.worst} \dash{} pobieranie, które zatrzymało się na ponad pięć
...
.NET tego nie robi, bo nic w .NET tego nie potrzebuje.
