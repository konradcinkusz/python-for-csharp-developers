...
the difference that bites is not thread safety, because CPython does take a
lock per module while a body runs and detects its own deadlocks, but that a
cycle lets a second module see this one half-built, which is
\S\ref{sec:cycles}.
\end{csbox}
...
\begin{csbox}
\code{-m} has no exact C\# counterpart, and the reason is the useful part.
In .NET what an assembly resolves against is declared \dash{} by the project,
and at run time by the deps file written beside it \dash{} so the directory
you happen to be standing in does not change which code loads. In Python it
does, because \api{sys.path} begins with a directory chosen by how you
started the process. The \code{src/} layout of Chapter~\ref{ch:packaging}
exists to stop that mattering, and this section is the mechanism underneath
it.
\end{csbox}
