...
timeouts are not a mistake you want to inherit: the read timeout is ten
minutes, which is a sensible default for a provider that streams a long
...
event's payload. This chapter settles it. A call is \emph{two} events and
not one, because a call that never came back is the case most worth being
able to find, and a single event written after the reply cannot represent
it. The payload carries the token counts and whether the reply parsed; it
does not carry the prompt or the completion, because a trace is kept,
shipped and read by people who were not in the room.
\end{projectbox}
