...
and it is equally true in C\#. What differs is the printing: Python shows
the shortest string that round-trips, so the discrepancy is on screen,
while C\# formats a \code{double} to fifteen significant digits by
default and hides it. Neither language is more accurate than the other. Use
\api{math.isclose} for computed floats, and \api{decimal.Decimal} for
money, which is the same advice as C\#'s \code{decimal}.
\end{trapbox}
