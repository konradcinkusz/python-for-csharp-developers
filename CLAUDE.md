...
| Chapters | **5 of 14 written: Chapter 3 *Typing*, Chapter 6 *Errors*, Chapter 7 *Imports and dependency injection*, Chapter 8 *asyncio*, and Chapter 10 *Data* — both editions** | 1, 2, 4, 5, 9, 11–14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, Chapters 3, 6, 7, 8 and 10's listings and exercises, E3, E4 and E6, and the measurement, ledger and transcript scripts, and CI runs all of it | every other chapter's listings and exercises; five of the eight experiments |
...
7, 8 and 10 were written in parallel, by separate passes, each against the
...
written chapter past; see the five pass notes below.
...
file of what parallel passes cost — five sessions each re-measured every
ledger, and one of them merged `main` three times, because each merge was
...
- 110 listing references, every file and region present · 23
  exercises, each with a starter, a solution and a test · 46 transcript
  references, every file present · 137 code files, none over 79 columns ·
...
- Parity: 23 file pairs, 0 failures, 0 warnings · 128 labels in each edition,
  0 mismatches
