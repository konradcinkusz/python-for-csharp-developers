...
| Chapters | **5 of 14 written: Chapter 3 *Typing*, Chapter 6 *Errors*, Chapter 7 *Imports and dependency injection*, Chapter 8 *asyncio* and Chapter 13 *The AI engineer's kit* — both editions** | 1, 2, 4, 5, 9–12, 14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton and its stage 02, Chapters 3, 6, 7, 8 and 13's listings and exercises, E3, E4 and E8, and the measurement, ledger and transcript scripts, and CI runs all of it | every other chapter's listings and exercises; every experiment the manifest's Status column still marks *not run* |
...
first one was written into a build that already had every gate. Those five
...
borrows little. The gates earned their keep on every one of them, and none had
to be loosened to let a written chapter past; see the pass notes below.
...
installation you measured on, every time — several sessions have now
measured this book on both font sets, so the rule has earned itself inside
one repository as well as between it and CI, and the check is
`kpsewhich newtxtext.sty` rather than an assumption about the image.
...
- 92 listing references, every file and region present · 22 exercises, each
  with a starter, a solution and a test · 44 transcript references, every
  file present · 129 code files, none over 79 columns · 19 pins agree between
...
- Parity: 23 file pairs, 0 failures, 0 warnings · 123 labels in each edition,
  0 mismatches · every written chapter inside its 3000-word prose budget,
  English and Polish: Chapter 3 at 2915 and 2516, Chapter 6 at 2638 and 2311,
  Chapter 7 at 2581 and 2242, Chapter 8 at 2269 and 1990, Chapter 13 at 2543
  and 2223
...
Five chapters of fourteen are written: 3, 6, 7, 8 and 13. The outstanding
work is tracked as GitHub issues under the `chapter`, `appendix`,
`experiment` and `infrastructure` labels — **work from the labels, not from a list here**,
...
1. **Chapters 1, 2, 4 and 5**, which with the written Chapters 3, 6 and 7
   ...
   each names what it borrows and borrows almost nothing. Treat the ordering as a
...
2. **Chapters 9 to 12** (v0.2), with the trace-assert stage 01 in Chapter 11
   and the experiments those chapters carry.
