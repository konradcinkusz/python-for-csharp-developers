...
| Chapters | **1 of 14 written: Chapter 7, *Imports and dependency injection*.** The other thirteen are briefs printed where the chapter will go | 1–6, 8–14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, Chapter 7's ten listings and four exercises, the two measurement scripts, and CI runs all of it | every other chapter's listings and exercises; the eight experiments |

**The scaffold carried every gate before a word was written**, which is what
it was for, and Chapter 7 is the first chapter written into it. Chapter 7 was
written before Chapters 1 to 6 because its issue came up first; nothing in it
depends on them being written, and the two dependencies its brief declares
(Chapters 2 and 5) are referred to rather than assumed.
...
- 24 listing references, every file and region present · 5 exercises, each
  with a starter, a solution and a test · 10 transcript references, every
  file present · 47 code files, none over 79 columns · 19 pins agree between
...
- Parity: 23 file pairs, 0 failures, 0 warnings · 46 labels in each edition,
  0 mismatches
...
1. **Chapters 1 to 6**, which with the written Chapter 7 complete v0.1.
   Suggested order: 1, 2, 3 first, because every later chapter's listings
   assume the reader trusts the environment; then 4, 5 and 6 in order. Note
   that Chapter 7 already refers forward to Chapter 2's `src/` layout and
   back to nothing, so it constrains none of them.
