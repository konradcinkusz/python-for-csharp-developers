...
| Chapters | **8 of 14 written: Chapter 2 *Environments and packaging*, Chapter 3 *Typing*, Chapter 6 *Errors*, Chapter 7 *Imports and dependency injection*, Chapter 8 *asyncio*, Chapter 10 *Data*, Chapter 12 *Observability and operations* and Chapter 13 *The AI engineer's kit* — both editions** | 1, 4, 5, 9, 11, 14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton and its stage 02, Chapters 2, 3, 6, 7, 8, 10, 12 and 13's listings and exercises, Chapter 12's three Dockerfiles, E2, E3, E4, E6 and E8, and the measurement, ledger and transcript scripts, and CI runs all of it | every other chapter's listings and exercises; every experiment the manifest's Status column still marks *not run* |

**The scaffold plus eight chapters.** The scaffold existed so that the shape
of the book could be argued with before any chapter was written, and so that
the first one was written into a build that already had every gate. All eight
...
`main` five and seven times, because each merge was overtaken before its own
...
which is deliberate and is what two passes independently arrived at: the
file is navigated by chapter, and a merge position is a fact about how a
note came to exist rather than about where to find it.
...
with a bare container. Two passes have compared the same source both ways,
...
- 148 listing references, every file and region present · 33 exercises, each
  with a starter, a solution and a test · 66 transcript references, every
  file present · 186 code files, none over 79 columns · 20 pins agree between
...
- 77 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 182 labels in each edition,
  0 mismatches
- Prose words against a budget of 3,000, and `csbox` translation boxes,
  ...
  re-read them from the tool rather than from here. **`reflist.py` reads the `.aux` tree**, so its label count is
...
Eight chapters of fourteen are written: 2, 3, 6, 7, 8, 10, 12 and 13. The
