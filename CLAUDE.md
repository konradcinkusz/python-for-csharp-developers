...
| Chapters | **5 of 14 written: Chapter 1 *CPython and the GIL*, Chapter 3 *Typing*, Chapter 6 *Errors*, Chapter 7 *Imports and dependency injection*, and Chapter 8 *asyncio* — both editions** | 2, 4, 5, 9–14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, Chapters 1, 3, 6, 7 and 8's listings and exercises, E1, E3 and E4, and the measurement, ledger and transcript scripts, and CI runs all of it | every other chapter's listings and exercises; five of the eight experiments |
...
7 and 8 were written in parallel, by separate passes, each against the issue
...
past; see the five pass notes below.
...
which the Chapter 1, Chapter 7 and Chapter 8 passes then paid for
independently: these two rows are from an installation that HAS `newtx`,
`inconsolata` and `tex-gyre` (`texlive-fonts-extra` plus `tex-gyre` and
`texlive-plain-generic`), where the scaffold's 37 and 37 and Chapter 3's 53
and 56 were from a bare one that degrades to `lmodern` through the
preamble's `\IfFileExists` probes. Different fonts, different line breaks,
different pagination. CI compiles on a full TeX Live; that is the inherited
two-machines rule and it is not a defect. Record which installation you
...
- 88 listing references, every file and region present · 21 exercises, each
  with a starter, a solution and a test · 40 transcript references, every
  file present · 124 code files, none over 79 columns · 19 pins agree between
...
- 69 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 116 labels in each edition,
  0 mismatches
- Word budgets, of 3000: `ch01-runtime` at 2399 English and 2098 Polish;
  ...
  `ch07-imports-di` at 2581 and 2242; `ch08-asyncio` at 2269 and 1990
...
- **The page counts in the Status table above changed provenance under
  ...
  **Several sessions measuring one book on two kinds of installation is now
  ...
  four-chapter one, and 102 against 102 on a five-chapter one. Polish prose
...
- **The exercise-key ordinal bit here too, which makes it four passes out of
  five.** The first draft had `\begin{exercise}{e01_02_...}` third in the
  ...
  passes broke is not one a fifth will keep. Chapter 7's note does not
  mention it, so whether that pass avoided it or merely never wrote an
  exercise out of order is not established here. It is
...
1. **Chapters 2, 4 and 5**, which with the written Chapters 1, 3, 6, 7 and 8
