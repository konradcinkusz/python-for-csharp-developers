...
| Chapters | **3 of 14 written: Chapter 3, *Typing*; Chapter 5, *Functions and control flow*; Chapter 7, *Imports and dependency injection* — both editions** | 1, 2, 4, 6, 8–14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, Chapters 3, 5 and 7's listings and exercises, the measurement scripts, and CI runs all of it | every other chapter's listings and exercises; the eight experiments |
...
3, 5 and 7 were written in parallel, by separate passes, each against the
issue that came up first rather than against the reading order — which is
possible because a chapter names what it borrows and borrows little. All
three met at merges rather than in sequence, so **the gate results below are
the merged tree's and not any one pass's**; the gates earned their keep on
each, and the three pass notes are below.
...
`kpsewhich newtxtext.sty` answers it in one line.
...
- 72 listing references, every file and region present · 14 exercises,
  each with a starter, a solution and a test · 32 transcript references,
  every file present · 92 code files, none over 79 columns · 19 pins
...
- Parity: 23 file pairs, 0 failures, 0 warnings · 93 labels in each edition,
  0 mismatches
...
1. **Chapters 1, 2, 4 and 6**, which with the written Chapters 3, 5 and 7
   ...
   Chapter 2 still has one to keep.
