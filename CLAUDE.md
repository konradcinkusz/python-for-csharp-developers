...
| Chapters | **6 of 14 written: Chapter 3 *Typing*, Chapter 5 *Functions and control flow*, Chapter 6 *Errors*, Chapter 7 *Imports and dependency injection*, Chapter 8 *asyncio*, and Chapter 10 *Data* — both editions** | 1, 2, 4, 9, 11–14 |
...
possible because a chapter names what it borrows and borrows little. All six
met at merges rather than in sequence, so **the gate results below are the
merged tree's and not any one pass's**. The gates earned their keep on each,
and none had to be loosened to let a written chapter past; see the six pass
notes below.
...
installation you measured on, every time — `kpsewhich newtxtext.sty` answers
it in one line, and see *Build traps* for the two `apt-get` lines that turn a
bare container into the reference one, which is what turns a page-level defect
from a guess-and-push loop into a verification loop. Chapter 10's pass
measured the same tree both ways and got 51 and 53 bare against 51 and 51
full, so `lmodern` cost the Polish edition two pages and the English none;
that is one controlled comparison and not a rule about which font sets
tighter.
...
- 144 listing references, every file and region present · 28 exercises,
  each with a starter, a solution and a test · 58 transcript references,
  every file present · 162 code files, none over 79 columns · 20 pins
...
- 52 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 158 labels in each edition,
  0 mismatches
...
1. **Chapters 1, 2 and 4**, which with the written Chapters 3, 5, 6 and 7
   complete v0.1. (Chapters 8 and 10 are written too, and are v0.2's.) The
   suggested order was 1, 2, 3 first, because every later
   ...
   two forward references, to Chapters 2 and 3: Chapter 3's is kept, and
   Chapter 2 still has one to keep. And two things that cost Chapter 6 a
