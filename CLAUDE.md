...
| Chapters | **4 of 14 written: Chapter 3, *Typing*, Chapter 6, *Errors*, Chapter 7, *Imports and dependency injection*, and Chapter 13, *The AI engineer's kit* — both editions** | 1, 2, 4, 5, 8–12, 14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton and its stage 02, Chapters 3, 6, 7 and 13's listings and exercises, the measurement scripts, and CI runs all of it | every other chapter's listings and exercises; every experiment the manifest's Status column still marks *not run* |
...
first one was written into a build that already had every gate. Those four
were written in parallel, by separate passes that did not know about each
...
borrows little. The gates earned their keep on every one of them; see the
pass notes below.
...
- 80 listing references, every file and region present · 17 exercises, each
  with a starter, a solution and a test · 34 transcript references, every
  file present · 106 code files, none over 79 columns · 19 pins agree between
...
- 36 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 107 labels in each edition,
  0 mismatches · every written chapter inside its 3000-word prose budget:
  Chapter 3 at 2915 and 2516, Chapter 6 at 2638 and 2311, Chapter 7 at 2581
  and 2242, Chapter 13 at 2543 and 2223, English and Polish
...
Four chapters of fourteen are written: 3, 6, 7 and 13. The outstanding work
is tracked as GitHub issues under the `chapter`, `appendix`, `experiment` and
...
4. **The experiments that have not run**, each free, each writing a value
   ...
   `notes/01-curriculum.md` §4 is the ledger, filled in by the pass that runs
   each one; naming them here would be a second copy to go stale, and no
   total is stated in either place.
