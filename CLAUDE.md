...
| Chapters | **6 of 14 written: Chapter 1 *CPython and the GIL*, Chapter 3 *Typing*, Chapter 6 *Errors*, Chapter 7 *Imports and dependency injection*, Chapter 8 *asyncio*, and Chapter 10 *Data* — both editions** | 2, 4, 5, 9, 11–14 |
...
ledger, and one of them merged `main` four times, because each merge was
...
- 120 listing references, every file and region present · 26 exercises, each
  with a starter, a solution and a test · 50 transcript references, every
  file present · 151 code files, none over 79 columns · 20 pins agree between
...
- 80 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 142 labels in each edition,
  0 mismatches
...
- **The page counts in the Status table above changed provenance under
  ...
  **Several sessions measuring one book on two kinds of installation is now
  ...
  four-chapter one, and 102 against 102 and 115 against 115 on trees of five
  and six. Polish prose
...
- **The exercise-key ordinal bit here too, which makes it four passes out of
  six.** The first draft had `\begin{exercise}{e01_02_...}` third in the
  ...
  passes broke is not one a seventh will keep. The Chapter 7 and Chapter 10
  notes do not mention it, so whether those passes avoided it or merely
  never wrote an exercise out of order is not established here. It is
