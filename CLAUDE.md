...
| Chapters | **6 of 14 written: Chapter 3 *Typing*, Chapter 6 *Errors*, Chapter 7 *Imports and dependency injection*, Chapter 8 *asyncio*, Chapter 9 *Services*, and Chapter 10 *Data* — both editions** | 1, 2, 4, 5, 11–14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, those six chapters' listings and exercises, experiments E3, E4, E5 and E6, and the measurement, ledger and transcript scripts, and CI runs all of it | every other chapter's listings and exercises; the experiments that have not run |
...
7, 8, 9 and 10 were written in parallel, by separate passes, each against
the issue that came up first rather than against the reading order — which
is possible because a chapter names what it borrows and borrows little. The
gates earned their keep on all six, and none had to be loosened to let a
written chapter past; see the pass notes below.
...
ledger, and more than one merged `main` several times, because each merge was
overtaken before its own CI could finish.
...
- 134 listing references, every file and region present ·
  28 exercises, each with a starter, a solution and a test ·
  54 transcript references, every file present · 162 code
...
- 74 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 152 labels in each edition,
  0 mismatches
...
1. **Chapters 1, 2, 4 and 5**, which with the written Chapters 3, 6 and 7
   ...
   chapter's listings assume the reader trusts the environment — and six
...
2. **Chapters 11 and 12** (v0.2), with the trace-assert stage 01 in
   Chapter 11 and experiment E7. Chapters 8, 9 and 10 are written, and E4,
   E5 and E6 with them.
...
4. **The experiments that have not run**, each free, each writing a value
   ...
   the ledger and says which; do not restate a total here. E4 is the worked
   example of committing bounds rather than timings, which is what makes a
   stopwatch survive `make verify`; E5 is the other shape, where the raw
   measurement is committed and the value file is derived from it.
