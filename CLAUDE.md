...
| Chapters | **4 of 14 written: Chapter 3, *Typing*, Chapter 6, *Errors*, Chapter 7, *Imports and dependency injection*, and Chapter 10, *Data* — both editions** | 1, 2, 4, 5, 8, 9, 11–14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, Chapters 3, 6, 7 and 10's listings and exercises, the measurement scripts, and CI runs all of it | every other chapter's listings and exercises; seven of the eight experiments |
...
7 and 10 were written in parallel, by separate passes, each against the issue
...
their keep on all four; see the four pass notes below.
**Two of those notes open by calling themselves the first chapter written**,
and neither is wrong from where it stood: the passes could not see each
...
file of what parallel passes cost — four sessions each re-measured every
ledger, and one of them had to merge twice because `main` moved underneath
it while its own CI was running.
...
to go stale. **And say which installation**, because there are two and they
paginate differently: the rows above are from one that HAS `newtx`,
`inconsolata` and TeX Gyre, which is what CI compiles on and therefore what
`pages.yml` publishes to a reader. A bare container degrades to `lmodern`
through the preamble's `\IfFileExists` probes, and sets the same source
differently: on the one tree measured both ways — Chapter 10's, before this
merge — a bare container gave 51 and 53 where a full TeX Live gave 51 and
51, so `lmodern` cost the Polish edition two pages and the English none.
Chapter 3's pass recorded 53 and 56 on a bare machine, but that was a
different tree and is not a second reading of the same thing. One
controlled comparison is not a rule about which font sets tighter, and no
direction is claimed here beyond what was measured. What is certain is that
the two disagree, which is the inherited two-machines rule and is not a
defect.
Record which installation you measured on, every time — and see *Build
traps* for how to turn a bare container into the reference one, which is two
`apt-get` lines and turns a guess-and-push loop into a verification loop.
...
- 98 listing references, every file and region present · 18 exercises, each
  with a starter, a solution and a test · 36 transcript references, every
  file present · 114 code files, none over 79 columns · 20 pins agree between
...
- 26 Mermaid sources, thirteen per language, all rendering, all placed
- 37 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 112 labels in each edition,
  0 mismatches
- Prose words against a budget of 3,000, and `csbox` translation boxes:
  Chapter 3 at 2,915 / 2,516 with 2 boxes, Chapter 6 at 2,638 / 2,311 with
  3, Chapter 7 at 2,581 / 2,242 with 3, Chapter 10 at 2,386 / 2,069 with 3
  — English first. These are the figures `--words` and `--csbox` print, and
  ...
  a fact about the last build and not about the source: rebuild before you
  quote it, or it reports the tree you had before the merge
...
1. **Chapters 1, 2, 4 and 5**, which with the written Chapters 3, 6 and 7
   ...
   writing the Polish.
2. **Chapters 8, 9, 11 and 12** (v0.2), with the trace-assert stage 01 in
   Chapter 11 and experiments E4, E5 and E7. Chapter 10 and E6 are done.
...
4. **The experiments that have not run**, each free, each writing a value
   ...
   `notes/01-curriculum.md` §4 says which those are; no count is stated
   here, because one goes stale the next time a pass runs one.
