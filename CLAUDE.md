...
| Chapters | **4 of 14 written: Chapter 2, *Environments and packaging*, Chapter 3, *Typing*, Chapter 6, *Errors*, and Chapter 7, *Imports and dependency injection* — both editions** | 1, 4, 5, 8–14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, Chapters 2, 3, 6 and 7's listings and exercises, the measurement scripts, and CI runs all of it | every other chapter's listings and exercises; the experiments that have not run |
...
6 and 7 were written in parallel, by separate passes, each against the issue
...
their keep on all four, and exactly one preamble change was needed, by the
Chapter 2 pass, whose own subject exposed it — see *The double-dash ligature*
below. The four pass notes are under *Resolved questions*.
...
- 74 listing references, every file and region present · 15 exercises, each
  with a starter, a solution and a test · 30 transcript references, every
  file present · 97 code files, none over 79 columns · 19 pins agree between
...
- Parity: 23 file pairs, 0 failures, 0 warnings · 99 labels in each edition,
  0 mismatches
...
Written in parallel with Chapters 3, 6 and 7, against separate issues, so it
...
Written into the scaffold in parallel with Chapters 2, 6 and 7, in sessions
...
Written into the scaffold in parallel with Chapters 2, 3 and 7, in sessions
...
Written into the scaffold in parallel with Chapters 2, 3 and 6, in sessions
...
4. **The experiments that have not run**, each free, each writing a value
   ...
   here, for the reason the ledger at the top of this file gives.
