...
| Chapters | **3 of 14 written: Chapters 2, 3 and 7 — both editions** | 1, 4–6, 8–14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, Chapters 2, 3 and 7's listings and exercises, the measurement scripts, and CI runs all of it | every other chapter's listings and exercises; the experiments that have not run |
...
their keep on all three, and exactly one preamble change was needed, by the
Chapter 2 pass, whose own subject exposed it — see *The double-dash ligature*
below. The three pass notes are under *Resolved questions*.
...
- 46 listing references, every file and region present · 11 exercises, each
  with a starter, a solution and a test · 24 transcript references, every
  file present · 77 code files, none over 79 columns · 19 pins agree between
...
- 30 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 76 labels in each edition,
  0 mismatches
...
Written in parallel with Chapters 3 and 7, against separate issues, so it is
not the first chapter into the scaffold and the heading no longer says it is:
three passes ran at once and the merge order decided which arrived first,
which is not a fact about the chapters. What is true of this one is that it is
the only one of the three that needed the preamble changed.
...
1. **Chapters 1, 4, 5 and 6**, which with the written Chapters 2, 3 and 7
