...
| Chapters | **2 of 14 written: Chapters 3 and 5.** The other twelve are briefs printed where the chapter will go | 1, 2, 4 and 6 to 14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, Chapter 3's six listings and four exercises, Chapter 5's eight listings and five exercises, four measurement scripts, and CI runs all of it | every other chapter's listings and exercises; the eight experiments |
...
that the first chapters were written into a build that already had every
gate. Chapters 3 and 5 are those chapters; they were written in parallel,
against separate issues, and met at a merge rather than in sequence — which
is why the gate results below are the merged tree's and not either pass's.
The other twelve still teach nothing.
...
to go stale. **And say which build**: the two rows above are from a machine
with `newtx` and `inconsolata` present, so the preamble's `\IfFileExists`
probes took the fonts rather than degrading to `lmodern`, and the log names
`ntxtlf` and `zi4`. That makes them comparable with CI's full TeX Live and
**not** comparable with a bare installation's — the Chapter 3 pass measured
53 and 56 on a bare one, and the two sets of figures are not a discrepancy
to reconcile. Check `kpsewhich newtxtext.sty` before quoting a page count;
that is the inherited two-machines rule and the divergence is not a defect.
...
- 50 listing references, every file and region present · 10 exercises,
  each with a starter, a solution and a test · 24 transcript references,
  every file present · 59 code files, none over 79 columns · 19 pins
...
- Parity: 23 file pairs, 0 failures, 0 warnings · 74 labels in each edition,
  0 mismatches
...
made here, because one instance is not yet a class. **The merge with
Chapter 3 is the second data point and it points the other way**: that
chapter's four keys were already in document order, so nothing was caught
by accident there — which is the argument for the check rather than
against it, since the two chapters differ only in whether somebody
happened to notice.
...
1. **Chapters 1, 2, 4, 6 and 7**, which complete v0.1. The suggested order
   was 1, 2, 3 first, because every later chapter's listings assume the
   reader trusts the environment. Chapters 3 and 5 were written first
   anyway, in parallel and out of that order, and between them they cost
   two things: Chapter 3 found in practice that it depends on neither 1
   nor 2 — it names what it borrows and borrows almost nothing — so the
   ordering is a preference rather than a constraint; and Chapter 5's two
   references forward to Chapters 2 and 3 are promises those chapters have
   to keep, of which Chapter 3's is now kept.
