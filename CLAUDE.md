...
| Chapters | **4 of 14 written: Chapter 3, *Typing*, Chapter 6, *Errors*, Chapter 7, *Imports and dependency injection*, and Chapter 9, *Services* — both editions** | 1, 2, 4, 5, 8, 10–14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, those four chapters' listings and exercises, the measurement scripts including experiments E3 and E5, and CI runs all of it | every other chapter's listings and exercises; the experiments that have not run |
...
7 and 9 were written in parallel, by separate passes, each against the issue
...
their keep on all four; see the pass notes below.
...
- 90 listing references, every file and region present ·
  18 exercises, each with a starter, a solution and a test ·
  34 transcript references, every file present · 112 code
...
- 55 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 110 labels in each edition,
  0 mismatches
...
and 7: the four sessions took whichever issue came up first, and none of the
four chapters needed any of the others. Chapter 9's measurement is free, its
...
Four chapters of fourteen are written: 3, 6, 7 and 9. The outstanding work is
