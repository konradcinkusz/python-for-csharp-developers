...
| Chapters | **6 of 14 written: Chapter 2 *Environments and packaging*, Chapter 3 *Typing*, Chapter 6 *Errors*, Chapter 7 *Imports and dependency injection*, Chapter 8 *asyncio*, and Chapter 10 *Data* — both editions** | 1, 4, 5, 9, 11–14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, Chapters 2, 3, 6, 7, 8 and 10's listings and exercises, E2, E3, E4 and E6, and the measurement, ledger and transcript scripts, and CI runs all of it | every other chapter's listings and exercises; the experiments that have not run |
...
possible because a chapter names what it borrows and borrows little. The
gates earned their keep on all six, and none had to be loosened to let a
written chapter past; the one change any of them forced was to the preamble,
by the Chapter 2 pass, whose own subject exposed it — see *The double-dash
ligature* below. The six pass notes are under *Resolved questions*.
**What parallel passes cost is visible in this file.** Several notes open by
saying where their own chapter stood among the ones their session could see,
and no two of those sentences agreed until a merge reconciled them; six
sessions each re-measured every ledger, and one of them merged `main` four
times, because each merge was overtaken before its own CI could finish. The
Chapter 2 note's opening is the one that says what follows from it: merge
order is not a fact about a chapter.
...
and not a rule about which font sets tighter.
...
- 118 listing references, every file and region present · 25 exercises, each
  with a starter, a solution and a test · 50 transcript references, every
  file present · 147 code files, none over 79 columns · 20 pins agree between
...
- 38 Mermaid sources, nineteen per language, all rendering, all placed
- 60 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 141 labels in each edition,
  0 mismatches
...
Written in parallel with Chapters 3, 6, 7, 8 and 10, against separate issues,
so it is not the first chapter into the scaffold and the heading no longer
says it is: six passes ran at once and the merge order decided which arrived
first, which is not a fact about the chapters. What is true of this one is
that it is the only one of the six that needed the preamble changed.
...
Written into the scaffold in parallel with Chapters 2, 6, 7, 8 and 10, in
...
Written into the scaffold in parallel with Chapters 2, 3, 7, 8 and 10, in
...
Written into the scaffold in parallel with Chapters 2, 3, 6, 8 and 10, in
...
Six chapters of fourteen are written. The outstanding work is tracked as
...
1. **Chapters 1, 4 and 5**, which with the written Chapters 2, 3, 6 and 7
   complete v0.1; Chapters 8 and 10 are v0.2's, written early. The suggested
   order was 1, 2, 3 first, because every later chapter's listings assume the
   reader trusts the environment — and six
