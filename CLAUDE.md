...
| Chapters | **4 of 14 written: Chapter 3, *Typing*, Chapter 6, *Errors*, Chapter 7, *Imports and dependency injection*, and Chapter 12, *Observability and operations* — both editions** | 1, 2, 4, 5, 8–11, 13, 14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, Chapters 3, 6, 7 and 12's listings and exercises, Chapter 12's three Dockerfiles, the measurement scripts, and CI runs all of it | every other chapter's listings and exercises; seven of the eight experiments |
...
7 and 12 were written in parallel, by separate passes, each against the issue
...
**And the one box in the table is the same effect, caught in the act.** It is
`2.3` pt in Chapter 7's §7.1, a paragraph of three unbreakable `\code{}` and
`\api{}` spans — the recorded class — and it is **not** a defect in that
chapter: this branch is byte-identical to `main` for every Chapter 7 file,
the log names `T1/lmr` as the only text font in the build, and Chapter 7's
own CI run reported zero overfull boxes on a full TeX Live. A run of unbreakable
spans that fits one measure does not fit another. **Do not fix a box you can
only see on a bare installation**; confirm against CI first, which is the
build that ships.
...
- 82 listing references, every file and region present · 17 exercises, each
  with a starter, a solution and a test · 34 transcript references, every
  file present · 107 code files, none over 79 columns · 19 pins agree between
...
- 33 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 106 labels in each edition,
  0 mismatches
...
Four chapters of fourteen are written, 3, 6, 7 and 12. The outstanding work is
...
1. **Chapters 1, 2, 4 and 5**, which with the written Chapters 3, 6 and 7
   ...
   writing the Polish.
2. **Chapters 8 to 11**, which complete v0.2 with the written Chapter 12.
   The trace-assert stage 01 is Chapter 11's, and experiments E4 to E6
   belong here. E7 is written and blocked on a machine that can reach a
