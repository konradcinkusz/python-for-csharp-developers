...
| Chapters | **5 of 14 written: Chapter 3 *Typing*, Chapter 6 *Errors*, Chapter 7 *Imports and dependency injection*, Chapter 8 *asyncio*, and Chapter 9 *Services* — both editions** | 1, 2, 4, 5, 10–14 |
| Appendices | **E (Manifest), generated.** A–D are briefs | A, B, C, D |
| Code | `code/` is a locked uv project: the `trace-assert` skeleton, those five chapters' listings and exercises, experiments E3, E4 and E5, and the measurement, ledger and transcript scripts, and CI runs all of it | every other chapter's listings and exercises; the experiments that have not run |
...
7, 8 and 9 were written in parallel, by separate passes, each against the
...
gates earned their keep on all five, and none had to be loosened to let a
...
- 102 listing references, every file and region present ·
  23 exercises, each with a starter, a solution and a test ·
  44 transcript references, every file present · 135 code
  files, none over 79 columns · 19 pins agree between `preamble.tex` and
  `code/pyproject.toml`
...
- 63 computed value keys, every one produced and every one used
- Parity: 23 file pairs, 0 failures, 0 warnings · 126 labels in each edition,
  0 mismatches
...
1. **Chapters 1, 2, 4 and 5**, which with the written Chapters 3, 6 and 7
   ...
   writing the Polish.
2. **Chapters 10, 11 and 12** (v0.2), with the trace-assert stage 01 in
   Chapter 11 and experiments E6 and E7. Chapters 8 and 9 are written, and
   E4 and E5 with them; Chapter 10 is the database session behind Chapter
   9's `get_store`, which that chapter names as handed on.
